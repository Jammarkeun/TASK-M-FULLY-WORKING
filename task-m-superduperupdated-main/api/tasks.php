...
switch ($method) {
    case 'GET':
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $task_item = [
                ...
                "category" => $category ?? 'uncategorized' // Default to 'uncategorized' if not set
            ];
    ...
    case 'PUT':
        ...
        if (isset($data->id)) {
            ...
            $task->status = $data->status ?? 'pending'; // Default if not set
            $task->priority = $data->priority ?? 'medium'; // Default if not set
            $task->category = $data->category ?? 'uncategorized'; // Default if not set
